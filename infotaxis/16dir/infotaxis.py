...
#            print("this_time: ", time.time()-start2)
            ...
            if new_agent_x <= 0 or new_agent_x >= 60 or new_agent_y <= 0 or new_agent_y >= 60:
                ...
                for z in range(num_interval):
                    ...
                    zu_sum.append(sum(zu_tot))
                    zWpnorm = zu_tot/zu_sum[z]
                    #print(sum(zWpnorm))

                    entropy_temp = zu_sum[z]*(-sum(zWpnorm*np.log2(zWpnorm+(zWpnorm==0)*1) ) +
                                               sum(Wpnorms*np.log2(Wpnorms+(Wpnorms==0)*1) ) )
                    if entropy_temp != entropy_temp:
                        print(entropy_temp)
                        entropy_temp = 0
                    entropy = entropy - entropy_temp
                entropy_a.append(entropy)
#            print("update_time: ", time.time()-start2)
        #print("entropy: ", entropy_a)
        #print(entropy_a)
