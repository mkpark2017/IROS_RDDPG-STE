...
#        last_action = last_action.permute(1,0,2)
        activation=F.relu
        # single branch
        x = torch.cat([state], -1).cuda(self.device)
        #print("forward device: ", x.device)
...
#            print("action_wrapped: ", action)
    ...
    def get_action(self, state, hidden_in,  noise_scale=1.0):
        ...
        state = torch.FloatTensor(state).unsqueeze(0).unsqueeze(0).cuda(self.device) # increase 2 dims to match with training data
#        last_action = torch.FloatTensor(last_action).unsqueeze(0).unsqueeze(0).cuda()
        normal = Normal(0, 1)
        action, hidden_out = self.forward(state, hidden_in)
        noise = noise_scale * normal.sample(action.shape).cuda(self.device)
        action= action + noise
        #print(action)
        #print(action[0])
        #print(action[0][0])
        ...
        return action.detach().cpu().numpy()[0][0], hidden_out.cuda(self.device)
