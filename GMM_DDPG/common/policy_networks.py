...
#        self.linear2 = nn.Linear(hidden_dim, hidden_dim)
        self.linear4 = nn.Linear(hidden_3, self._action_dim) # output dim = dim of action
    ...
    def forward(self, state):
        ...
        # single branch
        #x = torch.cat([state], -1)
        x = activation(self.linear1(state))   # lstm_branch: sequential data
...
#            print("action_wrapped: ", action)
    ...
    def get_action(self, state, noise_scale=1.0):
        ...
        '''
        state = torch.FloatTensor(state).unsqueeze(0).cuda(self.device) # increase 2 dims to match with training data
